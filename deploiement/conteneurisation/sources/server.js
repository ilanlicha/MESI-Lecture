...
app.get('/api/app', (req, res) => {
    ...
    if (id) {
        ...
        applications.findOne({ _id: id }, function (err, obj) {
            if (obj.status === "ON")
                exec('/appli/podman-remote/./podman container inspect -f \'{{.State.Status}}\' ' + obj.name.toLowerCase() + '_back ; /appli/podman-remote/./podman container inspect -f \'{{.State.Status}}\' ' + obj.name.toLowerCase() + '_front', (err, stdout, stderr) => {
                    if (stdout) {
                        console.log(stdout);
                        let count = stdout.split("running").length - 1;
                        if (count != 2)
                            applications.findOnnaneAndUpdate({ _id: id }, { status: "OFF" }, function (err, obj) {
                            });
                    } else {
                        console.log(stderr);
                        console.log(err);
        ...
        applications.find({}, function (err, obj) {
            obj.forEach(app => {
                if (app.status === "ON") {
                    exec('/appli/podman-remote/./podman container inspect -f \'{{.State.Status}}\' ' + app.name.toLowerCase() + '_back ; /appli/podman-remote/./podman container inspect -f \'{{.State.Status}}\' ' + app.name.toLowerCase() + '_front', (err, stdout, stderr) => {
                        if (stdout) {
                            console.log(stdout);
                        ...
                        if (obj.frontEnd === "" && obj.backEnd !== "") {
                            ...
                            if (isNaN(parseInt(obj.portFront)) && !isNaN(parseInt(obj.portBack))) {
                                ...
                                fs.writeFile(dir + "front_Dockerfile", obj.frontEnd, function (err) {
                                    ...
                                    exec('/appli/podman-remote/./podman build . -f /conteneurisation/data/files/' + id + '/tmp/front_Dockerfile -t ' + obj.name.toLowerCase() + '_front && /appli/podman-remote/./podman run -p' + obj.portFront + ':' + obj.portFront + ' --name ' + obj.name.toLowerCase() + '_front -d ' + obj.name.toLowerCase() + '_front', {
                                        cwd: dir
                                    }, (err, stdout, stderr) => {
                                        // lancement réussi
                                        if (stdout) {
                                            ...
                                            fs.writeFile(dir + "back_Dockerfile", obj.backEnd, function (err) {
                                                ...
                                                exec('/appli/podman-remote/./podman build . -f /conteneurisation/data/files/' + id + '/tmp/back_Dockerfile -t ' + obj.name.toLowerCase() + '_back && /appli/podman-remote/./podman run -p' + obj.portBack + ':' + obj.portBack + ' --name ' + obj.name.toLowerCase() + '_back -d ' + obj.name.toLowerCase() + '_back', {
                                                    cwd: dir
                                                }, (err, stdout, stderr) => {
                                                    // lancement réussi
                                                    if (stdout) {
...
app.put('/api/off', (req, res) => {
    ...
    try {
        applications.findOne({ _id: id }, { "name": 1 }, function (err, obj) {
            try {
                exec('/appli/podman-remote/./podman rm -f ' + obj.name.toLowerCase() + '_front && /appli/podman-remote/./podman rm -f ' + obj.name.toLowerCase() + '_back', (err, stdout, stderr) => {
                    if (stdout) {
                        console.log(stdout);
                        writeLog(id, stdout);
                        applications.findOneAndUpdate({ _id: id }, { status: "OFF" }, function (err, obj) {
